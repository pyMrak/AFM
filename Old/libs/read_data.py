...
def readDefFolder():
    file = open('Settings\defaultFolder.txt', 'r')
    i=0
    for line in file:
        folder = line
        i +=1
...
def readSettings(nr):
    file = open('Settings\program.txt', 'r')
    out = ''
...
def read_txt(file_name,path='',delimiter='.',maxlines=6502):
    if path=='':
        slash=''
    else:
        slash='\\'
    file=open(path+slash + file_name+'.txt', 'r')
    lines=[]
    count=0
    maxlines=maxlines-1
    for line in file:
    ...
    for i in range(maxlines-len(lines)+1):
        if i == 0:
            print('!!!Warning!!! Measurement file ' + file_name+'.txt does not contain enough lines. MaxLines is set to', maxlines)
...
class header():
    def __init__(self, date, order, production_date, nom_v, gp_nr='?unknown?',creator='Marko Kenda'):
        self.date=date
        self.order_nr=order
...
def read_TCdata(path,folder,files):
    TC_data=[]
    print('Reading data...')
    for file in files:
        TC_data.append(read_txt(file,path + folder,',',27506))
    print('Data read\n')     
...
class measurment_data():
    def __init__(self,TC=0,R=False,mark=False,MaxLines=6510,MeasDur=60,folder='measurements', graph=1, Curr=False, QCpath='local'):
        ...
        for i in range(26,60):
            self.columns.append(self.columns[math.floor(i/(26))-1]+self.columns[i%(26)])
        self.reset()
        self.readQCs()
    ...
    def read_data(self,folder=0):
        ...
        for file in self.filenames:
            self.data.append(read_txt(file,folder,',',self.maxlines))
            #print(file)
        ...
        for QCd in file.split('QC\n'):
            ...
            if QC+'>' in QCd or '<'+QC in QCd: # or QC+',' in QCd:
                ...
                try:
                    ...
                    print('!!!Warning!!! Not complete QC detected - Temperature to reach missing')
                    self.QC.TtR = 750
                ...
                try:
                    ...
                    if err:
                        print('!!!Warning!!! Not complete QC detected - Tolerances of time to reach temperature missing')
                ...
                try:
                    ...
                    if err:
                        print('!!!Warning!!! Not complete QC detected - Tolerances of Tmax missing')
                ...
                try:
                    ...
                    if err:
                        print('!!!Warning!!! Not complete QC detected - Tolerances of T60s missing')
                ...
                try:
                    ...
                    if err:
                        print('!!!Warning!!! Not complete QC detected - Tolerances of Imax missing')
                ...
                try:
                    ...
                    if err:
                        print('!!!Warning!!! Not complete QC detected - Tolerances of I60s missing')
                ...
                try:
                    ...
                    if err and self.R:
                        print('!!!Warning!!! Not complete QC detected - Tolerances of R missing')
