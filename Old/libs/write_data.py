...
def saveSettings(nr, newLine):
    file = open('Settings\program.txt', 'r')
    lines = file.readlines()
    file.close()
    lines[nr] = newLine + '\n'
    file = open('Settings\program.txt', 'w')
    for line in lines:
        file.write(line)
    file.close()
    
    
def save(gdat,file='xlsx', file_name="Analiza.xlsx"):
    if file=='xlsx':
        file = gdat.wb
#    elif file=='txt':
...
#            try:
                ...
                if graph_nr<1:
                    print('Graph nr. not correctly defined no graph written.')
                    break
                elif graph_nr == 1:
                    tag=''
                else:
                ...
                if g_nr>graph_nr-1:
                    ...
                    chart1.x_axis.scaling.max = round(gdat.MeasStart[i*10+j]/100)+gdat.MeasDur
...
def write_table(gdat):
    TCcol='A' ########################################pazi!!!!!
    print('Creating measurment table...')
    header = headers()
...
#                            break
    ...
    print('Measurment table created\n')
    return gdat
...
#    else:
#        path = gdat.path
    Anwb = load_workbook(gdat.folder + '\\' + label +'Analiza.xlsx') ####\\
    Aws = Anwb['Tabela']
    ...
    if gdat.folder == 'measurements':
        ...
        path = gdat.folder + '\\'
    dataTC=rd.measurment_data(TC=False,MaxLines=27010,MeasDur=270, folder=gdat.folder + '/TC800-1100')
    ...
    for i in range(gdat.length):
        ...
        if Aws['A'+str(i+2)].value=='OK' or Aws['A'+str(i+2)].value=='Ok' or Aws['A'+str(i+2)].value=='ok':
            ...
            TCwb['Poročilo'].add_image(Image(path + 'TCRTG\\' + gdat.filenames[i] + '.jpg', size=(170, 305)), 'D75')
...
class headers():
    ...
    def readHeaders(self):
        headerFile = open('settings/headers.txt')
