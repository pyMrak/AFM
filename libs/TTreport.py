...
class TTreportBuilder(object):
    ...
    def getTablesData(self, paths):
        ...
        for path in paths:
            ...
            if not appended:
                ...
                self.messageLogger.writeWarning('Skupina '+groupName+' nima datoteke s fun. meritvami oz je njeno ime nepravilno.', self.gdat, False)
        ...
        for tableData, group in zip(self.tablesData, self.settings.groups):
            if tableData is not None:
                if 'odpovedi' in tableData[0,-1].lower():
                    if tableData is not None:
                        if len(self.settings.groups) > 1:
                            self.createTable(document, array([tableData[:,0],tableData[:,-1]]), title='Tabela s cikli odpovedi skupine '+group+'.', integer=True)
                            tables += 1
                        else:
                            self.createTable(document, array([tableData[:,0],tableData[:,-1]]), title='Tabela s cikli odpovedi.', integer=True)
                            tables += 1
                else:
                    self.messageLogger.writeWarning('Skupina '+group+' nima definiranih odpovedi TT', self.gdat, False)
        return tables
    ...
    def createTable(self, document, tableData, title='', integer=False):
        ...
        tableStyle = [[parse_xml(r'<w:shd {} w:fill="1F5C8B"/>'.format(nsdecls('w'))) for i in range(len(tableData[0]))]]
    ...
    def addBoxPlot(self, document, x, labels, title, width):
        ...
        self.figN += 1
        plt.boxplot(x, labels=labels, patch_artist=True, medianprops={'color':'orange'})#, notch=None, sym=None, vert=None, whis=None, positions=None, widths=None, patch_artist=None, bootstrap=None, usermedians=None, conf_intervals=None, meanline=None, showmeans=None, showcaps=None, showbox=None, showfliers=None, boxprops=None, labels=None, flierprops=None, medianprops=None, meanprops=None, capprops=None, whiskerprops=None, manage_ticks=True, autorange=False, zorder=None, *, data=None)[source]
        plt.title(title)
    ...
    def addTwoBoxPlots(self, document, x, y, labelsX, labelsY, title, width):
        ...
        self.figN += 1
        ax1.boxplot(x, labels=labelsX, patch_artist=True, medianprops={'color':'orange'})#, notch=None, sym=None, vert=None, whis=None, positions=None, widths=None, patch_artist=None, bootstrap=None, usermedians=None, conf_intervals=None, meanline=None, showmeans=None, showcaps=None, showbox=None, showfliers=None, boxprops=None, labels=None, flierprops=None, medianprops=None, meanprops=None, capprops=None, whiskerprops=None, manage_ticks=True, autorange=False, zorder=None, *, data=None)[source]
    ...
    def createReport(self, openFile=False):
        ...
        if len(self.tablesData) == 0 or all(td is None for td in self.tablesData):
            ...
            pripravil.add_run(self.settings.author+', '+date).bold = False#document.add_paragraph(avtor, style='Podnaslov').bold = False
            ...
            for i in range(10):
                try:
                    ...
                    docFileName = self.settings.path+'/POROČILO O IZVEDENIH TRAJNOSTNIH TEKIH SVEČK ŽARILNIH TT-'+self.settings.TTnr+' '+month+year+'('+str(i+1)+').docx'
            if not saved:
                self.messageLogger.writeError('Dokumenta iz neznanega razloga ni mogoče shraniti.', self.gdat, False)
            elif i:
                self.messageLogger.writeWarning('Dokument z istim imenom je že odprt. Nova verzija dokumenta shranjena s končnico ('+str(i)+').', self.gdat, False)
...
class TTreportSettings(object):
    ...
    def getProcedura(self, group):
        if group in self.proceduraTT:
            ...
            self.messageLogger.writeWarning('Skupina '+group+' ne obstaja', self.gdat, False)#print('WARNING!: Skupina '+group+' ne obstaja')
            return ''
